...
func (c *Cluster) initSingleValidator(ctx context.Context, baseHome string) error {
	...
	if _, err := c.exec(ctx,
		"genesis", "add-genesis-account", "val", "1000000000000000umin",
...
func (c *Cluster) initMultiValidator(ctx context.Context, baseHome string, valCount int) error {
	...
	for i := 0; i < valCount; i++ {
		name := fmt.Sprintf("val%d", i)
		if _, err := c.exec(ctx,
			"genesis", "add-genesis-account", name, "1000000000000000umin",
			"--home", baseHome, "--keyring-backend", "test",
		); err != nil {
			return err
		}
...
func findRepoRoot() (string, error) {
	...
	for {
		if _, err := os.Stat(filepath.Join(current, "go.mod")); err == nil {
			...
			if readErr == nil && strings.Contains(string(modData), "github.com/initia-labs/minimove\n") {
				return current, nil
			}
