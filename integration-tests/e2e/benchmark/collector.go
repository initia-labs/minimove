...
func NewMempoolPoller(ctx context.Context, cl *cluster.Cluster, interval time.Duration) *MempoolPoller {
	if interval <= 0 {
	...
	for {
		...
		case <-ticker.C:
			...
			for i := 0; i < p.cl.NodeCount(); i++ {
				n, err := p.cl.UnconfirmedTxCount(ctx, i)
				if err != nil {
					continue
				}
				if n > maxSize {
...
// CollectResults iterates blocks from startHeight to endHeight and computes aggregate metrics
func CollectResults(
	ctx context.Context,
	cl *cluster.Cluster,
	cfg BenchConfig,
	...
	for h := startHeight; h <= endHeight; h++ {
		block, err := cl.QueryBlock(ctx, 0, h)
		if err != nil {
...
func CollectInitialMetas(ctx context.Context, cl *cluster.Cluster) (map[string]cluster.AccountMeta, error) {
	metas := make(map[string]cluster.AccountMeta)
	var mu sync.Mutex
	...
	for _, name := range cl.AccountNames() {
		...
		go func() {
			defer wg.Done()
			addr, err := cl.AccountAddress(name)
			if err != nil {
				mu.Lock()
				if firstErr == nil {
					firstErr = err
			...
			meta, err := cl.QueryAccountMeta(ctx, 0, addr)
			if err != nil {
				mu.Lock()
				if firstErr == nil {
					firstErr = err
