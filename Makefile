...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=minimove \
...
.PHONY: test test-all test-cover test-unit test-race benchmark benchmark-e2e
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./client/docs/statik/statik.go" -not -path "./tests/mocks/*" -not -name '*.pb.go' | xargs goimports -w -local github.com/initia-labs/minimove
.PHONY: format
